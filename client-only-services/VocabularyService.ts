...
class ClientVocabularyService {
  ...
  async getAllWordGroups(): Promise<WordGroup[]> {
    // Add learned status and mobile-compatible format
  ...
  async getAudioSettings(): Promise<UserSettings['audioSettings']> {
    return { ...this.settings.audioSettings };
  }
